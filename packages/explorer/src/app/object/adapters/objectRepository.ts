...
function parseMagstats(magstatsList: magstatsResponse[] | null): MagStatEntity[] {
  if ( magstatsList ) {  
...
function parseProbabilities(probabilitiesList: probabilitiesResponse[] | null): ProbabiilyEntity[] {
...
> = {
  parseTo: (
    ...
    const result: LightCurveEntity = {
      detections: parseDetections(response.detections),
      non_detections: parseNonDetections(response.non_detections)
    };
    return result;
  },
};
...
function parseNonDetections(nonDetectionsList: NonDetectionItem[]): NonDetectionEntity[] {
  return nonDetectionsList.map( (item) => {
    const nonDetectionEntity: NonDetectionEntity = {
      tid: item.tid,
      mjd: item.mjd,
      fid: item.fid,
      diffmaglim: item.diffmaglim
    };
    return nonDetectionEntity;
  })
}
